...
App({
    ...
    // 是否登录
    isLogin () {
        if (!wx.getStorageSync('accountInfo') || !wx.getStorageSync('stateType')) {
            that.showToast('您还未登录，请登录后再进行操作！','none');

            that.redirectTo('/pages/sellerBasic/authorize/index');
    ...
    request (url, data, callback) {
        util.httpRequest('POST', url, data).then(res => {
            ...
            that.showToast('请求出错', 'none');
        });
    },

    // 没有分页
    noPageHandle (callback) {
        callback();
    },

    // 有分页
    hasPageHandle (callback) {
        callback();
    },
